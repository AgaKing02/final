...
<div class="jumbotron">
    <div class="container">
        <h1 class="display-3">${group.getName()}</h1>
        <p>${group.getYear()}</p>
...
<div class="container">
    <c:if test="${group.getStudents()!=null}">
    ...
    <c:if test="${cookie.role.value=='ADMIN'}">
        <button id="${group.getId()}" onclick="getUnsignedUsers(this.id)" type="button" class="btn btn-info text-center" data-toggle="modal" data-target="#exampleModal1"
...
<script>
    ...
    function getUnsignedUsers(idd) {
        $.ajax({
            type: "GET",
            url: "http://localhost:8080/finalProjectAdvancedJava_war/getusers?groupid="+idd,
            cache: false,
            timeout: 600000,
    ...
    function addUsertoClub(idd,studentid) {
        $.ajax({
            type: "POST",
            url: "http://localhost:8080/finalProjectAdvancedJava_war/getusers?groupid="+idd+"&studentid="+studentid,
            cache: false,
            timeout: 600000,
            success: function (data) {
                alert("success")
    ...
    function removeUserfromClub(studentid) {
        ...
        $.ajax({
            type: "POST",
            url: "http://localhost:8080/finalProjectAdvancedJava_war/remove?groupid="+idd+"&studentid="+studentid,
            cache: false,
            timeout: 600000,
            success: function (data) {
                alert("success")
