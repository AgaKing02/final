...
<div class="jumbotron">
    <div class="container">
        <h1 class="display-3">${event.getEvent()}</h1>
...
<div class="container">
    <c:if test="${event.getListeners()!=null}">
        <table class="table table-striped">
            ...
            <tbody>
            <c:forEach items="${event.getListeners()}" var="student">
                <tr>
    ...
    <c:if test="${cookie.role.value=='ADMIN'}">
        <button id="${event.getId()}" onclick="getUnsignedUsers(this.id)" type="button" class="btn btn-info text-center" data-toggle="modal" data-target="#exampleModal1"
...
<script>
    ...
    function getUnsignedUsers(idd) {
        $.ajax({
            type: "GET",
            url: "http://localhost:8080/finalProjectAdvancedJava_war/getusers?eventid="+idd,
            cache: false,
            timeout: 600000,
    ...
    function addUsertoClub(idd,studentid) {
        $.ajax({
            type: "POST",
            url: "http://localhost:8080/finalProjectAdvancedJava_war/getusers?eventid="+idd+"&studentid="+studentid,
            cache: false,
            timeout: 600000,
            success: function (data) {
               alert("success")
    ...
    function removeUserfromClub(studentid) {
        ...
        $.ajax({
            type: "POST",
            url: "http://localhost:8080/finalProjectAdvancedJava_war/remove?eventid="+idd+"&studentid="+studentid,
            cache: false,
            timeout: 600000,
            success: function (data) {
                alert("success")
